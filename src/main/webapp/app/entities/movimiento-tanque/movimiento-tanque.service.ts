...
export class MovimientoTanqueService {
  ...
  queryByTanque(req?: any, tanqueId?: number): Observable<EntityArrayResponseType> {
    const options = createRequestOption(req);
    return this.http
      .get<IMovimientoTanque[]>(`${this.resourceUrl}/tanque/${tanqueId}`, { params: options, observe: 'response' })
      .pipe(map((res: EntityArrayResponseType) => this.convertDateArrayFromServer(res)));
  }
