...
export class TodoComponent implements OnInit, OnDestroy {
     ...
     */
    constructor(private _fuseSidebarService: FuseSidebarService, private _todoService: TodoService) {
        // Set the defaults
     ...
     */
    ngOnInit(): void {
        this._todoService.onSelectedTodosChanged.pipe(takeUntil(this._unsubscribeAll)).subscribe(selectedTodos => {
            setTimeout(() => {
        ...
        this._todoService.onFiltersChanged.pipe(takeUntil(this._unsubscribeAll)).subscribe(folders => {
            this.filters = this._todoService.filters;
        });

        this._todoService.onTagsChanged.pipe(takeUntil(this._unsubscribeAll)).subscribe(tags => {
        ...
        this._todoService.onCurrentTodoChanged.pipe(takeUntil(this._unsubscribeAll)).subscribe(([currentTodo, formType]) => {
            if (!currentTodo) {
