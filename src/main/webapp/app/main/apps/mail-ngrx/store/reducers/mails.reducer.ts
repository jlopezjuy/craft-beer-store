...
export function MailsReducer(state = MailsInitialState, action: MailsActions.MailsActionsAll): MailsState {
    switch (action.type) {
        ...
        case MailsActions.GET_MAILS_SUCCESS: {
            ...
            const entities = mails.reduce((_entities: { [id: number]: Mail }, mail: Mail) => {
                return {
                    ..._entities,
                    [mail.id]: mail
                };
        ...
        case MailsActions.SELECT_MAILS_BY_PARAMETER: {
            ...
            const arr = Object.keys(state.entities).map(k => state.entities[k]);
            const selectedMailIds = arr.filter(mail => mail[filter.parameter] === filter.value).map(mail => mail.id);
            return {
                ...state,
                selectedMailIds
            };
