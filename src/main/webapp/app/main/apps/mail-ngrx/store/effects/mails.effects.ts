...
export class MailsEffect {
    ...
    constructor(private actions: Actions, private mailService: MailNgrxService, private store: Store<State>) {
        this.store.pipe(select(getRouterState)).subscribe(routerState => {
            if (routerState) {
     ...
     */
    @Effect()
    getMails: Observable<MailsActions.MailsActionsAll> = this.actions.pipe(
        ...
        exhaustMap(action => {
            ...
            return this.mailService.getMails(handle).pipe(
                map((mails: Mail[]) => {
     ...
     */
    @Effect()
    updateMails: Observable<MailsActions.MailsActionsAll> = this.actions.pipe(
        ofType<MailsActions.UpdateMails>(MailsActions.UPDATE_MAILS),
        exhaustMap(action => {
            return forkJoin(action.payload.map(mail => this.mailService.updateMail(mail))).pipe(
                map(() => {
     ...
     */
    @Effect()
    setCurrentMail: Observable<Action> = this.actions.pipe(
     ...
     */
    @Effect()
    checkCurrentMail: Observable<Action> = this.actions.pipe(
     ...
     */
    @Effect()
    getMailsSuccess: Observable<MailsActions.MailsActionsAll> = this.actions.pipe(
        ofType<MailsActions.GetMailsSuccess>(MailsActions.GET_MAILS_SUCCESS),
        mergeMap(() => [new MailsActions.CheckCurrentMail()])
    );
    /**
     ...
     */
    @Effect()
    updateMailsSuccess: Observable<MailsActions.MailsActionsAll> = this.actions.pipe(
        ofType<MailsActions.UpdateMailsSuccess>(MailsActions.UPDATE_MAILS_SUCCESS),
        mergeMap(() => [new MailsActions.DeselectAllMails(), new MailsActions.GetMails()])
    );
    /**
     ...
     */
    @Effect()
    updateMailSuccess: Observable<MailsActions.MailsActionsAll> = this.actions.pipe(
     ...
     */
    @Effect()
    setFolderOnSelectedMails: Observable<MailsActions.MailsActionsAll> = this.actions.pipe(
     ...
     */
    @Effect()
    addLabelOnSelectedMails: Observable<MailsActions.MailsActionsAll> = this.actions.pipe(
