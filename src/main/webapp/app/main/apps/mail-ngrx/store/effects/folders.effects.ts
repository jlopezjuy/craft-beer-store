...
export class FoldersEffect {
    constructor(private actions: Actions, private mailService: MailNgrxService) {}
     ...
     */
    @Effect()
    getFolders: Observable<FoldersActions.FoldersActionsAll> = this.actions.pipe(
        ofType<FoldersActions.GetFolders>(FoldersActions.GET_FOLDERS),
        switchMap(action => {
            return this.mailService.getFolders().pipe(
                map((folders: any) => {
