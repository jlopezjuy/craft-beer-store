...
export class MailService implements Resolve<any> {
    ...
    resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<any> | Promise<any> | any {
        ...
        return new Promise((resolve, reject) => {
            Promise.all([this.getFolders(), this.getFilters(), this.getLabels(), this.getMails()]).then(() => {
    ...
    getFolders(): Promise<any> {
        return new Promise((resolve, reject) => {
            this._httpClient.get('api/mail-folders').subscribe((response: any) => {
    ...
    getFilters(): Promise<any> {
        return new Promise((resolve, reject) => {
            this._httpClient.get('api/mail-filters').subscribe((response: any) => {
    ...
    getLabels(): Promise<any> {
        return new Promise((resolve, reject) => {
            this._httpClient.get('api/mail-labels').subscribe((response: any) => {
    ...
    getMailsByFolder(handle): Promise<Mail[]> {
        return new Promise((resolve, reject) => {
            this._httpClient.get('api/mail-folders?handle=' + handle).subscribe((folders: any) => {
                const folderId = folders[0].id;

                this._httpClient.get('api/mail-mails?folder=' + folderId).subscribe((mails: any) => {
    ...
    getMailsByFilter(handle): Promise<Mail[]> {
        return new Promise((resolve, reject) => {
            this._httpClient.get('api/mail-mails?' + handle + '=true').subscribe((mails: any) => {
                this.mails = mails.map(mail => {
                    return new Mail(mail);
                });
    ...
    getMailsByLabel(handle): Promise<Mail[]> {
        return new Promise((resolve, reject) => {
            this._httpClient.get('api/mail-labels?handle=' + handle).subscribe((labels: any) => {
                const labelId = labels[0].id;

                this._httpClient.get('api/mail-mails?labels=' + labelId).subscribe((mails: any) => {
                    this.mails = mails.map(mail => {
                        return new Mail(mail);
                    });
    ...
    updateMail(mail): Promise<any> {
        return new Promise((resolve, reject) => {
            this._httpClient.post('api/mail-mails/' + mail.id, { ...mail }).subscribe(response => {
